...
const SprintViewScreen = () => {
  const { resourceAllocationsApi, phaseApi } = useLambdasApi();
  const {
    filterType,
    searchQuery,
    selectedProject,
  ...
  const [projectPhasesMap, setProjectPhasesMap] = useState<
    Record<string, { severaPhaseId: string; name: string }[]>
  >({});
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const projectPhases: Record<string, { severaPhaseId: string; name: string }[]> = {};
        for (const allocation of allocations) {
          const projectId = allocation.project?.severaProjectId;
          if (projectId && !projectPhases[projectId]) {
            const phases = await phaseApi.getPhasesBySeveraProjectId({
              severaProjectId: projectId
            });
            projectPhases[projectId] = phases.map((p) => ({
              severaPhaseId: p.severaPhaseId || "",
              name: p.name || "Unnamed Phase"
            }));
          }
        }
        setProjectPhasesMap(projectPhases);

        // Initialize filterType so row clicks work
    ...
    fetchData();
  }, [loggedInUser, adminMode, resourceAllocationsApi, phaseApi, setError, setFilterType]);

  // Apply search/project filter + phase filter
  const filteredAllocations = resourceAllocations.filter((allocation) => {
    const matchesPhase = !selectedPhaseId || allocation.phase?.severaPhaseId === selectedPhaseId;
    const matchesSearch = filterAllocations([allocation], adminMode).length > 0;
          ...
          {adminMode && (
            <>
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: 2, flexWrap: "wrap" }}>
                ...
                {/* Phase filter */}
                <FormControl sx={{ minWidth: 140 }}>
                  <InputLabel id="phase-select-label">{strings.sprint.phase}</InputLabel>
                  <Select
                  ...
                  >
                    ...
                    {Object.values(projectPhasesMap)
                      .flat()
                      .map((phase) => (
                        <MenuItem key={phase.severaPhaseId} value={phase.severaPhaseId}>
                          {phase.name}
                        </MenuItem>
                      ))}
                  </Select>
