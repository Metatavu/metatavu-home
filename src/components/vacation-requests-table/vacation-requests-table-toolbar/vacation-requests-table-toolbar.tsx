...
const TableToolbar = ({
	...
	const [toolbarFormMode, setToolbarFormMode] = useState<ToolbarFormModes>(
		ToolbarFormModes.NONE,
	);
	const [confirmationHandlerOpen, setConfirmationHandlerOpen] = useState(false);
	const [editConfirmationHandlerOpen, setEditConfirmationHandlerOpen] =
		useState(false);
	const [editVacationsData, setEditVacationsData] =
	...
	const buttonLabel = isUpcoming
		? strings.tableToolbar.future
		: strings.tableToolbar.past;
	const singleSelectionSize = adminMode ? 3 : 4;
	const multiSelectionSize = adminMode ? 6 : 12;
	const gridItemSize =
		selectedRowIds?.length === 1 ? singleSelectionSize : multiSelectionSize;
						...
						<>
							...
							{isDraftSelected && !adminMode && (
								<ToolbarGridItem xs={4}>
									<ToolbarSubmitButton
										onClick={() =>
											handleSubmitForApproval(selectedRowIds[0] as string)
										}
									/>
								</ToolbarGridItem>
							)}
				...
				<ToolbarGridContainer container>
					...
					<ToolbarGridItem item xs={3}>
						...
						>
							...
							{!adminMode && (
								<MenuItem value="DRAFT">{strings.tableToolbar.draft}</MenuItem>
							)}
